...
logging.basicConfig(level=logging.WARNING, filename='display.log', filemode='w', format='%(asctime)s - %(levelname)s - %(message)s')
    ...
    try:
        while True:
            try:
                ...
                ret = ser.read(2)
                if ret[0] != 0x00 or (ret[1] not in [0x14, 0xf1, 0xf9]):

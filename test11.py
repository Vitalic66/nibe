...
def send_message(serial_mark, serial_space, message_bytes):
    ...
    try:
        ser_m = serial.Serial(serial_mark, 19200, bytesize=serial.EIGHTBITS, stopbits=serial.STOPBITS_ONE, parity=serial.PARITY_MARK, timeout=3)
        ser_s = serial.Serial(serial_space, 19200, bytesize=serial.EIGHTBITS, stopbits=serial.STOPBITS_ONE, parity=serial.PARITY_SPACE, timeout=3)

        # Warte auf die erwartete Sequenz
        if wait_for_sequence(ser_m, ser_s):
            ...
            ser_s.flush()
            print(f"📤 Gesendete Nachricht mit PARITY_SPACE: {message_bytes.hex(' ')}")
            ser_s.write(bytes.fromhex("06"))
            ser_s.flush()
            print(f"📤 Gesendet mit PARITY_SPACE auf {serial_port_write}: 06")
            ser_m.write(bytes.fromhex("03"))
            ser_m.flush()
            print(f"📤 Gesendet mit PARITY_MARK auf {serial_port_write}: 03")
...
def main():
    ...
    parser.add_argument("--port_mark", type=str, default="/dev/ttyUSB0", help="Serieller Port (Standard: /dev/ttyUSB0)")
    parser.add_argument("--port_space", type=str, default="/dev/ttyUSB1", help="Serieller Port für das Schreiben (Standard: /dev/ttyUSB1)")
